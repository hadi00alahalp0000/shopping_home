...
import 'package:shopping_home/serves/add_product_to_my_order_serves.dart';
import 'package:shopping_home/serves/check_valida_token_serves.dart';

class AddProductToMyOrderController extends GetxController {
  // Serves
  CheckValidTokenServes checkToken = CheckValidTokenServes();
  AddProductToMyOrderServes service = AddProductToMyOrderServes();

  // Values
  late String check;
  late String checkResponse;
  static var quantities = 1;
  ...
  Future<void> addProductToMyOrder(int id) async {
    ...
    if (check == CasesFromServer.can) {
      checkResponse = await service.addProductToMyOrder(
